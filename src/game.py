...
class SnakeGame:
    ...
    def reset(self):
        self.snake = [(self.board_size // 2 + i, self.board_size // 2) for i in
                      range(1, -2, -1)]  # Initialize the snake with three cells in (row, column) format.
        self.non_snake = set([(row, col) for row in range(self.board_size) for col in range(self.board_size) if
                              (row, col) not in self.snake])  # Initialize the non-snake cells.
    ...
    def step(self, action):
        ...
        # If snake eats food, it won't pop the last cell.
        # The food grid will be taken by snake later, no need to update board vacancy matrix.
    ...
    def draw_snake(self):
        ...
        pygame.draw.circle(self.screen, (255, 255, 255), (head_x + self.cell_size - eye_offset, head_y + eye_offset),
                           eye_size)

        # Draw the body (color gradient)
        color_list = np.linspace(255, 100, len(self.snake), dtype=np.uint8)
        i = 1
        body_x, body_y, body_width, body_height, body_radius = None, None, None, None, None
